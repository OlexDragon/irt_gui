...
import irt.data.NumericChecker;
...
import irt.data.packets.interfaces.Register;
...
//									alert.getDialogPane().setMinHeight(200);
	...
	@Override protected void setPacket(String propertiesKeyStartWith) throws PacketParsingException {
		...
		final int addr = IrtGuiProperties.getLong(propertiesKeyStartWith + "memory", Long.valueOf(address==0 ? 0x10+2 : 0x10+3)).intValue();
		logger.error(addr);
		RegisterValue rv = new RegisterValue(index, addr); //0x10+2 --> RDAC:MEM2; 0x10+3 --> RDAC:MEM3
