...
//		System.out.println("Read - "+Arrays.toString(readBytes));
	...
	public boolean wait(int eventValue, int waitTime) throws SerialPortException {
		...
		while(isOpened() && !(isReady = getInputBufferBytesCount()>=eventValue) && t<eventValue && isRun()){
			synchronized (this) {

				try { wait(waitTime); } catch (InterruptedException e) {}

//				Console.appendLn(""+waitTime+"; isSerialPortEven="+isSerialPortEven, "wait");
				if(isSerialPortEven){
