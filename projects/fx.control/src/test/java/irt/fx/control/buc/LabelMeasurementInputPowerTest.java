...
public class LabelMeasurementInputPowerTest extends ApplicationTest {
	...
	private LabelMeasurementInputPowerFx inputPower;
	...
	public void start(Stage stage) throws Exception {
		...
		try {
			...
			inputPower = new LabelMeasurementInputPowerFx();
			inputPower.setKey(PREFS_NAME);

			stage.setScene(new Scene(new VBox(comboBoxSerialPortFx, inputPower)));
			stage.show();
	...
	public void toolkitTest(){
		final Tooltip tooltip = inputPower.getTooltip();
	...
	public void valueChangeTest() throws InterruptedException, ExecutionException, TimeoutException {
		...
		if(text.equals("Label")){
			...
			inputPower.addValueChangeAction(statusChangeAction);
			logger.trace(text);

			ft.get(3, TimeUnit.SECONDS);

			assertNotEquals(text, inputPower.getText());
		}
